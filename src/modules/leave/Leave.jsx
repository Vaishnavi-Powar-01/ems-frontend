...
const Leave = () => {
  ...
  const validateDates = () => {
    ...
    if (fromDate < today) {
      setDateError("From date cannot be in the past. Please select today's date or future date.");
      return false;
    }
    if (toDate < fromDate) {
  ...
  const resetForm = () => {
    setFormData({ leave_type: "", from_date: "", to_date: "", reason: "", document: null });
    setDateError("");
  };
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage your leave requests and history</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
            <div className="text-gray-500 text-sm">Total Leaves</div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center p-6 border-b">
                ...
                <button onClick={() => { setIsModalOpen(false); resetForm(); }} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
              <form onSubmit={handleSubmit} className="p-6">
                <div className="space-y-4">
                  ...
                  {formData.from_date && formData.to_date && !dateError && (
                    <div className="bg-blue-50 p-3 rounded-lg">
                      <p className="text-sm text-blue-700">
                        Total Days: <strong>{calculateDays(formData.from_date, formData.to_date)}</strong> day(s)
                      </p>
                    </div>
                  )}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Upload Medical Certificate / Leave Letter
                    </label>
                    <input
                      type="file"
                      name="document"
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  {["Leave Type","From Date","To Date","Days","Reason","Document","Admin Comment","Status"].map((h) => (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {leaves.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="px-6 py-12 text-center text-gray-500">No leave records found</td>
                  </tr>
                ) : (
                  leaves.map((leave) => (
                    <tr key={leave.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        {/* ✅ FIX: document is now a full Cloudinary URL — use directly */}
                        {leave.document ? (
                          <a
                            href={leave.document}
                            target="_blank"
                      ...
                      <td className="px-6 py-4">{leave.admin_comment || "—"}</td>
